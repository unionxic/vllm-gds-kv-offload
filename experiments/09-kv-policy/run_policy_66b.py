"""09: 07의 결합 구성에 KV 정책 축을 더한 러너. run_combo_66b.py와 측정 절차는 동일하고 정책 스위치만 추가.
   정책 두 계열:
     staging admission — expfs 스펙 내장. --kv-transport cufile_staged|posix_staged 와 --staging-policy 로 켠다.
       block(포화 시 대기) / skip(포화 시 저장 생략) / cpu_fallback(포화 시 D2H 후 CPU write) / value(hint 기반).
       skip·cpu_fallback·value 는 read 진행 중 ring store 를 비차단 처리하는 게이트를 함께 켠다.
     제출 스케줄링 — lib/policies.py 몽키패치. --policy read_priority|deferred_store.
       deferred_store 는 요청 gap 에서만 배출하므로 generate 호출 사이에 on_request_gap 을 부른다.
   FIX=blocking_event 환경변수로 store 스레드의 CUDA event 스핀 제거(lib/prof_fix.py)도 켤 수 있다.
   usage: python run_policy_66b.py --kv-transport cufile_staged --staging-policy cpu_fallback --tag ..."""
...
ap.add_argument("--staging-slots", type=int, default=2)      # chunk 141MiB x slots 만큼 GPU 를 더 잡는다
ap.add_argument("--staging-writers", type=int, default=2)
ap.add_argument("--cpu-fallback-slots", type=int, default=4)
ap.add_argument("--max-outstanding-write-mb", type=int, default=0)   # 0 이면 상한 없음
ap.add_argument("--policy", default="none", choices=["none", "read_priority", "deferred_store"])
ap.add_argument("--value-mode", default="value_density", choices=["random_skip", "seen_twice", "value_density", "oracle"])
...
OUT = args.out_dir or os.path.join(ROOT, "results", "combined", "opt66b"); os.makedirs(OUT, exist_ok=True)
...
if args.kv_transport != "none":
    ...
    if args.kv_transport.endswith("_staged"):
        kw["kv_transfer_config"].kv_connector_extra_config.update({
            ...
            "expfs_max_outstanding_write_bytes": (args.max_outstanding_write_mb << 20) or None})
    import expfs
    ...
    # staged 는 write 가 write_slot 으로 나가므로 따로 센다(안 감싸면 kv_write_n 이 0 으로 죽는다)
    _ws0 = expfs.StagedCuFileTransport.write_slot
    def _ws_cnt(self, slot, path, kind="ring"):
        cnt["kv_write_n"] += 1; cnt["kv_write_b"] += self.chunk_bytes
        return _ws0(self, slot, path, kind)
    expfs.StagedCuFileTransport.write_slot = _ws_cnt
    if os.environ.get("FIX"):
        import prof_fix; prof_fix.install()
    if args.policy != "none":
        import policies; policies.install(args.policy)
...
for rnd in (1, 2):
    ...
    t1 = time.time(); out1 = llm.generate(prompts, sp1)
    if args.policy == "deferred_store": policies.on_request_gap(worker_holder)
    pre = time.time() - t1
    t2 = time.time(); outN = llm.generate(prompts, spN)
    if args.policy == "deferred_store": policies.on_request_gap(worker_holder)
    tot = time.time() - t2
...
if args.kv_transport != "none":
    _w = getattr(__import__("expfs"), "LAST_WORKER", None)
    if _w is not None and hasattr(_w.transport, "flush"):
        _t = time.time(); _w.transport.flush()   # 잔여 비동기 write 를 벽시계에 포함(생략 아님을 증명)
        res["staged_flush_s"] = round(time.time() - _t, 2)
        res["staged_stats"] = dict(getattr(_w.transport, "stats", {}))
        res["staged_write_errors"] = getattr(_w.transport, "write_errors", 0)
if args.policy != "none":
    import policies; res["policy_summary"] = policies.summary()
...
summary = {k: v for k, v in res.items() if k in ("load_s", "tiers", "ids_equal_rounds", "gpu_max_gib", "kv_files", "kv_bytes_gib", "staged_stats", "staged_flush_s", "policy_summary")}
