...
for f in $FRACS; do for a in $ARMS; do for r in $REPS; do
  tag="${a:0:1}-h${f}-r${r}"; [ "$NSYS" = 1 ] && tag="${tag}-nsys"
  [ -f $OUT/$tag.json ] && { echo "skip $tag (exists)"; continue; }
  free_gb=$(df --output=avail -BG / | tail -1 | tr -dc 0-9); [ "$free_gb" -lt 100 ] && { echo "disk free ${free_gb}G < 100G, abort"; exit 1; }
  echo "== $tag $(date +%H:%M:%S)"
  if [ "$NSYS" = 1 ]; then
    /usr/local/cuda/bin/nsys profile -t cuda,nvtx,osrt --cuda-memory-usage=false --sample=none --cpuctxsw=none \
      ...
      python run_66b.py --transport $a --host-fraction $f --tag $tag > $OUT/$tag.log 2>&1
  else
    python run_66b.py --transport $a --host-fraction $f --tag $tag > $OUT/$tag.log 2>&1
  fi
