...
print("|---|---|---|---|---|---|---|---|---|---|---|")
SLOW = 1.08  # 같은 arm 최소 load_s 대비 8% 이상 느린 런 = 시스템 지연 이상치(런 전체가 느림) → 중앙값에서 제외
outl = []
for arm, rs in rows.items():
    lo = min(r["load_s"] for r in rs)
    slow = [r for r in rs if r["load_s"] > lo * SLOW]
    if slow and len(rs) > 1:
        outl += [f"{arm}: " + ",".join(f"load {r['load_s']:.0f}s/decode {r['decode_step_s']:.1f}s" for r in slow)]
        rs = [r for r in rs if r not in slow]
...
if outl: print("excluded slow-system outliers:", "; ".join(outl))
