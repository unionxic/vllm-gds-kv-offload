...
case "$mode" in
  nsys)
    ...
    PROF_INSTRUMENT=1 nsys profile \
      ...
      -o "results/bailian/prof/${id}" --force-overwrite=true \
      python harness/run_bench.py --run-id "${id}" "$@" \
  ...
  pyspy)
    PROF_INSTRUMENT=1 ~/miniconda3/envs/gdsllm/bin/py-spy record \
      ...
      -o "results/bailian/prof/${id}.speedscope.json" -- \
      python harness/run_bench.py --run-id "${id}" "$@" \
...
echo "done: results/bailian/prof/${id}*"
