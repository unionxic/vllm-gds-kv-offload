...
for f in ${RATIOS:-0.5}; do for cond in ${CONDS:-none cufile cufile-wb}; do tag=ram$f-$cond${TAGSUF:-}
  [ -f $O/$tag/result.json ] && { log "skip $tag"; continue; }
  case $cond in none) kvt=none; extra=();; cufile) kvt=cufile; extra=();; cufile-wb) kvt=cufile; extra=(--kv-extra "$WB");; *) log "unknown cond $cond"; continue;; esac
  ...
  for util in 0.9 0.85; do
    "${wrap[@]}" python run_obs.py --run-dir $O/$tag --model $MODEL --prompt-source longbench --n-docs ${NDOCS:-32} --decode-tokens ${DECODE:-8} --max-model-len ${MAXLEN:-12288} \
