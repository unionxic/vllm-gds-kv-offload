...
| results/model-host-baseline | 기준표 결과 json과 로그, campaign.log, RAM 0.1 점(ram0.1), 폴링 검증(poll0, poll1) |
...
- 저장 라운드 추가 시간은 SSD 티어가 클수록 커져 host 0.1에서 44~72초. 66B와 같은 디스크 공유. 두 라운드 합계로는 30b가 host 1.0과 0.7에서 −5.2%와 −2.9%, 13b는 0.7에서 −3.4%만 이득이고 host 0.3 이하는 전 모델에서 손해. 6.7b는 host 1.0 외에는 손해이며 그 대부분이 아래 폴링 지연.
...
- 66B RAM 0.5, 0.6, 0.7, 0.8은 가중치 비율로 환산(0.516, 0.620, 0.723, 0.826)해 별도 측정. 0.5는 CPU 33 / SSD 31 layer, forward 23.86 / 23.84 s, 적중 라운드 −6.1%, 저장 +28 s, 두 라운드 합계 −1.3%. 0.6은 39 / 25 layer, 21.40 / 21.28 s, −6.9%, +15 s, 합계 −2.4%. 0.7과 0.8은 결과 json(opt-66b-ram0.7-*, ram0.8-*)이 생기는 대로 tools/baseline_table.py로 채울 것.
- 0.7은 CPU 46 / SSD 18 layer, forward 18.54 / 18.29 s, 적중 라운드 −7.9%, 저장 +24 s, 두 라운드 합계 −2.1%.
- 저장 라운드의 디스크 공유를 nvme0n1 1초 샘플(/proc/diskstats)로 직접 확인(RAM 0.7 적중 런 1라운드). 배치마다 KV 8.3 GiB 쓰기가 10~12초 동안 712~855 MB/s로 나가고, 그 초들에서 가중치 읽기가 3,213 MB/s에서 223~383 MB/s로 떨어지며 읽기 지연이 6.3 ms에서 13~17 ms로 오름. 쓰기가 없는 초는 읽기 3.2 GB/s, 지연 6.3 ms. 저장 손해가 SSD 몫에 비례하는 이유가 이 구간의 가중치 읽기 정체.
- 마무리 남은 일. 66B RAM 0.7과 0.8 두 쌍, prefetch_step 2 한 쌍(opt-66b-ram0.7-step2-*, 전송과 계산 겹침 확인, 예상 prefill 35 s에서 22.6 s), 저장 라운드 중 nvme0n1 1초 샘플(읽기·쓰기 MB/s와 읽기 지연, 스크래치 diskstat.log)로 디스크 공유를 직접 확인. 결과가 쌓이면 이 절의 표와 README 측정 6을 갱신.
