...
66B RAM 0.7에서 배치 2로는 다섯 번 시도해 모두 실패. KV 10.4 GiB + 버퍼 4.08 GB는 OOM, KV 9.2 GiB로 줄이면 두 세트의 등록이 BAR1을 채워 bounce 매핑 실패(등록 상한 100 MiB로 해결), PYTORCH_CUDA_ALLOC_CONF=expandable_segments는 cuMemMap 주소라 cuFile의 cuMemcpyPeer가 INVALID_VALUE(쓰지 말 것), 그 뒤에도 요청 2개 prefill의 fc1 활성화 290 MB가 안 들어감. 16 GB 카드에서 버퍼 두 세트와 요청 2개분 KV는 같이 못 넣는다. 배치 1(KV 5.2 GiB), 재계산만, 한 라운드로 비교.
