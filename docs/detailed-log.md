...
### 세 경로 KV 공급 실험 (sunny 추론 노드, rain 원격 노드)

이 절부터는 새 연구 갈래다. 요약은 README, 아래는 환경·측정·정정의 상세 기록이며 표의 수치는 results/의 result.json·requests.jsonl에서 나왔다.
