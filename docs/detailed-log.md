...
- 조각을 키워도 벽시계는 디스크 한계에 붙고 CPU만 8 MiB에서 28% 감소. ring 단독(17%)보다 큼.
...
반복 요청 첫 토큰 중앙값 32초(재계산), 55초(게이트 없음), 29초(게이트 2). 손해가 이득으로 전환. 예측값 430초와 일치. 수준 1은 재사용과 1회성이 섞인 배치에서 무효(1회성 요청은 로드가 없어 대상이 아님). 수준 2를 맞추는 데 네 번 실패했고 각각 스케줄러 내부 동작 하나씩을 남김. 비동기 admit된 요청은 그 step 동안 지역 임시 큐에 있어 정식 큐 검사가 못 봄. 커넥터 적중 조회가 비동기라 첫 응답이 "아직 모름". 스케줄러가 큐를 훑으며 요청을 옮기므로 로드 중 여부는 큐 위치가 아니라 요청 ID 집합으로 추적. 검사를 조회보다 앞에 두면 재사용 요청까지 잡혀 로드가 하나씩만 시작되고 배치가 1, 1, 2, 2, 3, 3, 5로 흘러 들어감. 로드는 병렬로 시작하게 두고 계산만 미뤄야 함.
...
- seen_twice는 라운드 1에 저장을 안 해 라운드 2가 재계산과 같고 실제로 읽는 라운드 3에서 같은 33초 손해. 손해가 절반이 된 것은 읽기를 피한 덕분이지 원인을 고친 것이 아님. 디스크는 39.4 GiB 대 3.9 GiB로 10분의 1이지만 쓰기가 공짜라 벽시계에 안 나타남.
...
디스크는 39.4 GiB에서 19.7 GiB로 절반이 됐지만 벽시계는 라운드당 1.4초 느려짐. 게이트 뒤 남은 로드 대기가 배치당 1초 남짓이라 바이트를 줄여 얻을 상한이 그 정도였고 변환 비용이 상쇄. 출력 품질은 미측정(입력이 난수 토큰). 양자화는 디스크 용량이 제약일 때의 수단이지 이 환경의 지연 개선 수단은 아님.
...
시간은 단조 시계로 벽시계, getrusage로 CPU 시간. 엔진 step을 직접 돌려 step마다 출력 요청 수로 prefill과 decode를 분류하고 1초 미만 step은 폴링으로 분리. nsys는 확인용으로 한 번(NVTX 표식으로 step, 라운드, KV 읽기와 쓰기), 기록 170 MB. 경로 확인은 cuFile TRACE 로그, nvidia-fs 통계, dmesg.
...
교훈. 1 MiB cuFile 조각 구성은 시스템 상태에 민감해 단독 측정으로 결론 내지 말 것. 실행 중인 셸 스크립트는 편집하지 말 것(bash가 읽는 중). pkill -f 패턴에 자기 명령줄이 매칭되지 않게 대괄호를 쓸 것. 실험 폴더에 보고서를 따로 두지 말고 이 문서와 README에만 적을 것.
...
| host | CPU / SSD 티어 | forward 실측 / 모형 | prefill 재계산 / 적중 | 배치당 로드 대기 | 라운드 재계산 / 적중 | 저장 라운드 추가 |
|---|---|---|---|---|---|---|
| 0.7 | 44층 83.5 GiB / 20층 38.0 GiB | 19.4 / 19.1 s | 35.0 / 19.7 s | 2.7 s | 683 / 633 s (−7.3%) | +6 s |
| 0.5 | 31층 58.9 GiB / 33층 62.7 GiB | 24.7 / 24.7 s | 40.3 / 25.1 s | 3.4 s | 854 / 807 s (−5.4%) | +25 s |
| 0.3 | 19층 36.1 GiB / 45층 85.4 GiB | 29.8 / 29.8 s | 45.3 / 30.3 s | 2.9 s | 1,015 / 971 s (−4.4%) | +45 s |
| 0.1 | 6층 11.4 GiB / 58층 110.1 GiB | 35.7 / 35.4 s | 51.2 / 36.0 s | 3.0 s | 1,205 / 1,153 s (−4.3%) | +66 s |
...
- 재사용이 한 번이면 host 0.3 이하에서 저장 손해와 적중 이득이 상쇄된다. host 0.7에서는 이득이 손해의 8배. 가중치의 SSD 몫이 클수록 KV 오프로드의 순이익이 줄고 원인은 디스크 공유.

#### opt-6.7b, 13b, 30b, host 1.0에서 0.1

같은 설계로 작은 모델 세 개. 13b와 30b는 정적 버퍼 등록 상한 100 MiB(아래 BAR1 항목). host 1.0은 SSD 없이 GPU+CPU.

| 모델 | host | CPU / SSD layer | forward 실측 / 모형 | prefill 재계산 / 적중 | 배치당 로드 대기 | 라운드 wall clock 재계산 / 적중 | 저장 라운드 추가 |
|---|---|---|---|---|---|---|---|
| 6.7b | 1.0 | 32 (12.0 GiB) / 0 | 1.12 / 1.05 s | 3.52 / 1.14 s | 0.8 s | 45.6 / 39.3 s (−14.0%) | +0.6 s |
| 6.7b | 0.7 | 22 (8.2) / 10 (3.8) | 2.23 / 1.89 | 4.50 / 2.20 | 5.6 | 80.7 / 92.5 (+14.6%) | +0.3 |
| 6.7b | 0.5 | 15 (5.6) / 17 (6.4) | 2.99 / 2.48 | 5.25 / 3.01 | 4.2 | 104.8 / 112.3 (+7.1%) | +5.1 |
| 6.7b | 0.3 | 9 (3.4) / 23 (8.6) | 3.62 / 2.99 | 5.80 / 3.59 | 4.9 | 125.0 / 133.0 (+6.5%) | +7.8 |
| 6.7b | 0.1 | 3 (1.1) / 29 (10.9) | 4.21 / 3.49 | 6.35 / 4.19 | 2.3 | 143.1 / 142.6 (−0.3%) | +44.3 |
| 13b | 1.0 | 40 (23.4) / 0 | 2.18 / 2.05 | 6.36 / 2.24 | 4.6 | 86.6 / 88.1 (+1.7%) | +1.6 |
| 13b | 0.7 | 28 (16.4) / 12 (7.0) | 3.73 / 3.63 | 7.82 / 3.83 | 1.3 | 136.0 / 125.5 (−7.8%) | +1.0 |
| 13b | 0.5 | 19 (11.1) / 21 (12.3) | 4.92 / 4.81 | 9.01 / 5.04 | 2.3 | 174.2 / 168.0 (−3.5%) | +8.2 |
| 13b | 0.3 | 11 (6.5) / 29 (17.0) | 5.96 / 5.87 | 10.03 / 6.02 | 1.8 | 207.2 / 197.0 (−4.9%) | +18.6 |
| 13b | 0.1 | 4 (2.3) / 36 (21.1) | 6.87 / 6.79 | 10.91 / 6.99 | 2.2 | 236.4 / 229.8 (−2.8%) | +71.6 |
| 30b | 1.0 | 48 (55.1) / 0 | 5.08 / 4.81 | 13.08 / 5.20 | 2.9 | 194.9 / 174.5 (−10.4%) | +4.7 |
| 30b | 0.7 | 33 (37.9) / 15 (17.2) | 8.92 / 8.69 | 16.74 / 9.01 | 2.8 | 317.3 / 295.6 (−6.9%) | +3.1 |
| 30b | 0.5 | 23 (26.4) / 25 (28.7) | 11.37 / 11.27 | 19.18 / 11.54 | 4.3 | 395.7 / 382.7 (−3.3%) | +11.2 |
| 30b | 0.3 | 14 (16.1) / 34 (39.0) | 13.71 / 13.59 | 21.53 / 14.09 | 2.4 | 470.7 / 455.8 (−3.2%) | +32.8 |
| 30b | 0.1 | 4 (4.6) / 44 (50.5) | 16.65 / 16.18 | 24.43 / 16.78 | 3.0 | 564.9 / 543.3 (−3.8%) | +51.6 |
...
- 저장 라운드 추가 시간은 SSD 티어가 클수록 커져 host 0.1에서 44~72초. 66B와 같은 디스크 공유.

#### 마지막 배치의 KV 로드 지연과 엔진 폴링

6.7b와 13b의 적중 라운드에서 배치 1~3의 로드 대기는 0.6~0.9초인데 배치 4만 5~21초. 66B에서도 배치 4가 2.5초에서 3.2초로 늘어 방향이 같음. 배치 4의 읽기 하나가 0.07초에서 중앙값 2.2초, 최대 9.3초로 늘고, 그 구간의 엔진 폴링(출력 없는 step)이 초당 2,500회에서 4,000회로 빨라진 것이 같이 잡힘. 대기 요청이 남아 있으면 step에 할 일이 있어 루프가 느슨하고, 마지막 배치에서는 할 일이 없어 루프가 조여지며 같은 프로세스의 KV 읽기 스레드가 GIL을 못 잡는 구조(01의 GIL 콘보이와 같은 기전).

검증. 러너에 출력 없는 step 뒤 1 ms 양보 옵션(--poll-sleep-ms)을 넣고 6.7b host 0.7 적중을 양보 있음과 없음으로 재실행.

| 조건 | 배치별 로드 대기 | 폴링 step | KV 읽기 스레드 점유 합 | 2라운드 wall clock | 재계산 80.7 s 대비 |
|---|---|---|---|---|---|
| 양보 없음 (원 런) | 0.6, 0.6, 0.6, 20.6 s | 88,584 | 151 s | 92.5 s | +14.6% |
| 양보 없음 (대조 재실행) | 0.8, 0.6, 0.6, 14.4 s | 63,774 | 125 s | 91.6 s | +13.5% |
| 1 ms 양보 | 0.6, 0.6, 0.6, 0.6 s | 937 | 17 s | 72.3 s | −10.4% |

- 원인은 디스크가 아니라 엔진 루프의 GIL 독점. 6.7b에서 적중이 손해로 나온 세 칸은 전부 이 지연이고, 양보 하나로 10% 이득으로 뒤집힘. 66B는 읽기 조각이 144 MiB라 파이썬 왕복이 적어 영향이 작음.
- 6.7b host 1.0을 SSD layer 없이 재실행했을 때 지연이 사라진 것(대기 0.8초)은 가중치 SSD 읽기 스레드까지 GIL을 다투는 상황에서만 굶주림이 심해진다는 뜻으로 읽히나, 직접 측정은 아님.
- 기준표의 다른 런은 비교 가능성을 위해 양보 없이 둠. 실제 vLLM의 엔진 루프도 원격 KV를 기다리는 동안 같은 방식으로 도므로 고칠 지점은 러너가 아니라 스케줄러 쪽. 미구현.
...
- step 개수 상한 10만이 위험했음. 폴링 step이 초당 4천 개라 6.7b host 0.7 적중 런은 한 라운드에 88,616까지 갔고, 넘었으면 요청이 미완료인 채 결과가 저장됐을 것. 넘은 런은 없음을 전 결과에서 확인. wall clock 3시간 상한으로 바꾸고 걸리면 기록.
- step의 prefill과 decode 라벨. 예전 기준(첫 토큰을 기다리는 요청이 하나라도 있으면 prefill)은 배치가 여럿일 때 decode forward를 prefill로 적어 decode로 잡힌 것이 마지막 배치뿐이었음. 09의 "decode 구간 KV 읽기 겹침 0초"는 그 범위에서 계산된 것. 첫 토큰이 나온 step만 prefill로 잡도록 바꾸고, step 원자료로 전 런을 다시 집계한 결과 decode 28 step 기준으로도 겹침은 0초.
- 폴링 양보 sleep을 step 시간 측정 밖으로.
