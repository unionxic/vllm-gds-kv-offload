...
# ---------------------------------------------------------------------------
# Leak lifecycle — SIGKILL / crash resilience
# ---------------------------------------------------------------------------


def _mp_hold_region(engine_id: str, ready_queue) -> None:
    """Create a region, report readiness, then hold it until killed."""
    region = SharedOffloadRegion(
        engine_id=engine_id,
        num_blocks=2,
        rank=0,
    ...
    ready_queue.put(region.mmap_path)
    while True:  # parent SIGKILLs us; cleanup() must never run
        time.sleep(60)


def _spawn_and_sigkill_holder(engine_id: str) -> str:
    """Start a child holding a region, SIGKILL it, return the region path."""
    ctx = get_mp_context()
    ready_queue = ctx.Queue()
    child = ctx.Process(target=_mp_hold_region, args=(engine_id, ready_queue))
    child.start()
    path = ready_queue.get(timeout=60)
...
def test_sigkill_orphan_reclaimed_by_next_start(iid):
    """A region orphaned by SIGKILL must be reclaimed when any later engine
    constructs its own region (no engine can clean up after a hard kill, so
    the responsibility falls on the next start)."""
    orphan_path = _spawn_and_sigkill_holder(f"{iid}-victim")
    assert os.path.exists(orphan_path), "SIGKILL must leave the file behind"
    try:
        with _region(f"{iid}-next"):
            assert not os.path.exists(orphan_path), (
                "next engine start must reclaim the orphaned region"
            )
    finally:
        _cleanup_file(orphan_path)


def test_sigkill_same_engine_id_restart(iid):
    """Restarting with the same engine_id after a hard kill must yield a fresh
    region whose creator removes the file on clean shutdown (no lasting leak)."""
    orphan_path = _spawn_and_sigkill_holder(iid)
    try:
        r = _make_region(iid, num_blocks=2)
        assert r.mmap_path == orphan_path
        r.cleanup()
        assert not os.path.exists(orphan_path), (
            "clean shutdown after a same-id restart must remove the file"
        )
    finally:
        _cleanup_file(orphan_path)


def test_live_region_survives_other_engine_start(iid):
    """A living region (held by another process) must NOT be reclaimed when a
    different engine starts."""
    ctx = get_mp_context()
    ready_queue = ctx.Queue()
    child = ctx.Process(target=_mp_hold_region, args=(f"{iid}-live", ready_queue))
    child.start()
    live_path = ready_queue.get(timeout=60)
    try:
        with _region(f"{iid}-other"):
            assert os.path.exists(live_path), (
                "a live region owned by another process must not be reclaimed"
            )
    finally:
        child.kill()
        child.join(timeout=10)
...
def test_init_failure_creator_leaves_no_file(iid, monkeypatch):
    """If __init__ fails after the creator made the file, the file must not
    leak (best-effort removal on the exception path)."""
    import vllm.v1.kv_offload.cpu.shared_offload_region as sor

    def boom(*args, **kwargs):
        raise RuntimeError("injected mmap failure")

    monkeypatch.setattr(sor.mmap, "mmap", boom)
    path = f"/dev/shm/vllm_offload_{iid}.mmap"
    try:
        with pytest.raises(RuntimeError, match="injected"):
            _make_region(iid, num_blocks=2)
        assert not os.path.exists(path), (
            "creator must remove the file when construction fails"
        )
    finally:
        _cleanup_file(path)


def test_same_id_zero_length_orphan_recovered(iid):
    """A zero-length orphan at OUR OWN path (creator died between O_EXCL
    create and ftruncate) must not wedge the engine_id: a same-id restart
    reclaims it and becomes a fresh creator instead of waiting forever in
    _wait_for_file_size."""
    path = f"/dev/shm/vllm_offload_{iid}.mmap"
    fd = os.open(path, os.O_CREAT | os.O_EXCL | os.O_RDWR, 0o600)
    os.close(fd)  # 0-byte, no flock holder — a wedged creator corpse
    try:
        start = time.monotonic()
        r = _make_region(iid, num_blocks=2)
        assert time.monotonic() - start < 5, "must not spin in _wait_for_file_size"
        assert r._creator, "restart must become a fresh creator"
        r.cleanup()
        assert not os.path.exists(path)
    finally:
        _cleanup_file(path)
...
def test_ftruncate_failure_leaves_no_file(iid, monkeypatch):
    """A creator whose ftruncate fails must remove its empty file."""
    def boom(fd, size):
...
def test_reclaim_skips_zero_length_file(iid):
    """A zero-length file is a region mid-creation (between O_EXCL create and
    ftruncate) and must not be reclaimed by a starting engine."""
    mid_creation = f"/dev/shm/vllm_offload_{iid}-mid.mmap"
    fd = os.open(mid_creation, os.O_CREAT | os.O_EXCL | os.O_RDWR, 0o600)
    try:
        os.close(fd)
        with _region(f"{iid}-starter"):
            assert os.path.exists(mid_creation), (
                "zero-length (mid-creation) files must be left alone"
            )
    finally:
        _cleanup_file(mid_creation)
