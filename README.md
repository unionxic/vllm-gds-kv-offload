...
- 미해결: 이중 버퍼(prefetch_step 2) 조건에서 KV 오프로드의 손익(16 GB 카드에서는 배치 2와 같이 못 넣어 미측정). deferred_store로 저장 손해를 줄일 수 있는지. cuFile Batch API 엔진 통합. cuFile 1 MiB 조각 경로가 간헐적으로 3배 느려지는 모드의 원인. 스케줄러 게이트의 이득이 2.5%로 작고 GPU 한 장이라 일반성 미확립. 06의 host 0.1 편차 68%는 4 MiB 조각 재측정에서 재현되지 않아 1 MiB 느린 모드로 봄.
