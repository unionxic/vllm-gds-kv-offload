...
표의 V2 GDS 수치는 재현 시도 세 번에서 유지되지 않았고(tail이 두 배로 악화, 현행 tiering보다 못함), cuFile 단독 효과도 transport 대조에서 중립이었다. 그 tail 변동의 원인은 이후 규명됐다. store 실행이 foreground 엔진 구간과 겹치는 것 자체가 문제이며(GIL 콘보이), store를 요청 사이 gap으로 미루면 tail 5배, CPU 12배 개선이 변동계수 0으로 재현되고 이 효과는 cuFile과 POSIX에서 동일하다. 결론: SSD 티어는 유효하고, 승부처는 transport가 아니라 store 스케줄링이다. 부수 성과로 vLLM 버그 둘(요청 종료 레이스, /dev/shm mmap 누출)을 찾아 후자를 flock 기반 회수로 근본 수정했다.
