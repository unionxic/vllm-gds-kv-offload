...
이 저장소는 rain 서버(RTX 5000 16GB, 로컬 NVMe, native GDS 스택)에서 진행한 실험의
마스터 기록이다. 본 README가 설계·결과·판정의 단일 정리본이고, 각 디렉터리는 해당
단계의 실행 코드와 측정 산출물(CSV·로그)을 담는다. 대용량 재생성 데이터(trace 원본,
kvroot, 벤치 바이너리)는 .gitignore로 제외된다.

| 경로 | 내용 |
|---|---|
| `env.sh` | 공통 실행 환경 (conda libstdc++ preload, PYTHONHASHSEED) |
| `phase0/` | 기능 개통: fs 티어 스모크, FS-hit 분리 증명(fs_hit.py), GPU 레이아웃 판정(check_layout.py) |
| `phase05/` | SSD KV 정당성 게이트: A(재계산)/B(CPU hit)/C(SSD hit) 3군 벤치 |
| `phase1/` | vLLM 밖 cuFile 마이크로벤치(gdslib.py=ctypes 래퍼, cufile_bench.py, path_classify.py) |
| `phase2/` | out-of-tree GDS filesystem 스펙 expfs.py(ExperimentalFilesystemSpec)와 스모크 |
| `phase3/` | A~E 비교군 벤치, nsys 프로파일 스크립트, GIL 진단 |
| `w1/` | Bailian coder trace 리플레이(replay.py), causal-closure 하네스, /dev/shm E2E 시험 |

관련 vLLM 로컬 패치(shm mmap 누출 근본 수정)는 `~/vllm` 브랜치 `offload-shm-leak-fix`
(커밋 07bb9458eb + 77c4033078)에 있다.
...
실제 시스템·trace·선행 연구를 분류하고 현재 vLLM fs 티어와 의미론이 맞는 대표 workload를
...
- 추천 워크로드 (2.5개):
  1. Mooncake×Bailian coder/thinking (사실상 한 몸): production validity 최고(근거 1).
     ...
     명시한 ongoing work(DRAM hop 제거)의 선행 검증이라는 위치까지 확보.
...
1. 2차 티어는 스케줄러 프로세스에서 실행 (`tiering/base.py:106`) — CUDA 컨텍스트/GPU 텐서 없음.
...
GPU 쪽 연속성은 모델·attention backend·model runner에 따라 달라진다.
...
함의: GDS 이득은 V1 러너(cross-layer) 또는 큰 blocks_per_chunk와 결합할 때 실현되고,
