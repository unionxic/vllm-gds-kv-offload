...
class PrefetchOffloadConfig:
    ...
    offload_ssd_path: str | None = None
    """Directory for the SSD tier. When set, offloaded layers are placed in
    pinned host memory only until `offload_host_fraction` of DRAM is used;
    the remaining offloaded layers are stored as files under this directory
    and streamed into GPU each forward. Requires enforce_eager (the reads are
    host-driven and cannot be captured into CUDA graphs)."""

    offload_host_fraction: float = Field(default=0.3, ge=0.0, le=1.0)
    """Fraction of total host DRAM (MemTotal) that the pinned host tier may
    use when `offload_ssd_path` is set. Default 0.3."""

    offload_ssd_transport: Literal["cufile", "posix"] = "cufile"
    """How SSD-tier files reach the GPU: "cufile" = GPUDirect Storage
    (file -> GPU, no host hop), "posix" = O_DIRECT pread into a pinned bounce
    buffer followed by an H2D copy (file -> CPU -> GPU)."""

    offload_ssd_io_threads: int = Field(default=4, ge=1)
    """Number of IO threads reading SSD-tier files (per layer, parameters are
    read in parallel)."""

    offload_ssd_ring_mb: int = Field(default=0, ge=0)
    """cufile transport only. Size in MiB of each slot of a small registered
    GPU ring (2 x io_threads slots): cuFileRead lands in a slot by direct DMA
    and a D2D copy moves it into the static buffer. Useful when the static
    buffers are too large to register (BAR1 limit). 0 disables the ring."""
...
class OffloadConfig:
    ...
    def validate_offload_config(self) -> "OffloadConfig":
        ...
        if self.prefetch.offload_ssd_path and self.prefetch.offload_group_size <= 0:
            raise ValueError(
                "offload_ssd_path requires the prefetch backend "
