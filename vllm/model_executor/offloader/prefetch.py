...
from vllm.model_executor.offloader.ssd_tier import (
    SsdFile,
    SsdTier,
...
# Parameters smaller than this stay in pinned host memory even inside an
# SSD-tier layer (biases, norms): not worth a file handle / unaligned IO.
SSD_MIN_PARAM_BYTES = 1 << 20

logger = init_logger(__name__)


class IoWindow:
    """지금 가중치를 SSD에서 실제로 읽는 중인지 알리는 스레드 안전 신호.

    KV 오프로드 backend(cufile_fs)가 이 신호를 보고 같은 NVMe에 대한 KV 쓰기를 미룬다(write-behind).
    판정은 SSD 티어 읽기 스레드가 cuFileRead/pread에 들어가고 나오는 시점(activity)으로 한다.
    prefetch 발행 시점(mark)은 실제 읽기보다 수 초 앞설 수 있어 NVTX 표시에만 쓴다.
    연속한 layer 읽기 사이의 짧은 빈틈은 grace_ns 동안 SSD 구간으로 유지한다.
    """

    def __init__(self, grace_ns: int = 50_000_000):
        self.ssd_busy = threading.Event()  # 관측·호환용(activity 기준으로 set/clear)
        self._lock = threading.Lock()
        self._inflight = 0
        self._last_zero_ns = 0
        self.grace_ns = grace_ns
        self.last_change_ns = time.monotonic_ns()

    def mark(self, mode: str) -> None:
        """prefetch 발행 시점의 티어(NVTX 표시 전용)."""
        torch.cuda.nvtx.mark(f"prefetch_issue:{mode}")

    def activity(self, start: bool) -> None:
        with self._lock:
            if start:
                self._inflight += 1
                if self._inflight == 1:
                    self.last_change_ns = time.monotonic_ns()
                    self.ssd_busy.set()
                    torch.cuda.nvtx.mark("ssd_window:on")
            else:
                self._inflight -= 1
                if self._inflight == 0:
                    self._last_zero_ns = time.monotonic_ns()

    def is_ssd_window(self) -> bool:
        with self._lock:
            if self._inflight > 0:
                return True
            if time.monotonic_ns() - self._last_zero_ns < self.grace_ns:
                return True
            if self.ssd_busy.is_set():
                self.ssd_busy.clear()
                self.last_change_ns = time.monotonic_ns()
                torch.cuda.nvtx.mark("ssd_window:off")
            return False


# Pinned host storage without the caching-host-allocator power-of-two rounding.
# torch.empty(..., pin_memory=True) goes through CachingHostAllocator, which
# rounds every block up to PowerOf2Ceil(size): OPT-66B layers (fc1/fc2 679 MB
# -> 1 GiB each) end up occupying ~1.45x their nominal bytes. Register a
# pageable buffer with cudaHostRegister instead (exact size, page aligned).
_registered_pinned: list[torch.Tensor] = []


def _pinned_exact(size, stride, dtype, layout) -> torch.Tensor:
    assert layout == torch.strided, "only strided tensors are supported"
    numel = 1 + sum((d - 1) * st for d, st in zip(size, stride) if d > 0) if len(size) else 1
    nbytes = max(numel * torch.empty((), dtype=dtype).element_size(), 1)
    # device="cpu" is mandatory: the loader may run under a torch.device("cuda")
    # context, which would silently put this buffer on the GPU.
    buf = torch.empty(nbytes, dtype=torch.uint8, device="cpu")
    # Register from the storage base: Tensor.is_pinned() looks up the storage
    # pointer, so an offset-aligned registration would report "not pinned".
    # cudaHostRegister page-aligns the range internally.
    err = torch.cuda.cudart().cudaHostRegister(buf.data_ptr(), nbytes, 0)
    if int(err) != 0:
        raise RuntimeError(
            f"cudaHostRegister(ptr=0x{buf.data_ptr():x}, {nbytes} B) failed: "
            f"code={int(err)}; registered so far={len(_registered_pinned)}"
        )
    _registered_pinned.append(buf)
    return torch.as_strided(buf.view(dtype), size, stride)


def _empty_host(size, stride, dtype, layout, pin_memory: bool) -> torch.Tensor:
    if pin_memory and envs.VLLM_OFFLOAD_PIN_EXACT:
        return _pinned_exact(size, stride, dtype, layout)
    return torch.empty_strided(
        size=size, stride=stride, dtype=dtype, layout=layout, device="cpu",
...
class PrefetchOffloader(BaseOffloader):
    ...
    def __init__(
        ...
        host_fraction: float = 0.3,
        ssd_transport: str = "cufile",
        ...
        self._prefetch_plan: list[list[int]] = []  # built in post_init
        self.offload_params = offload_params or set()
        self.mode = mode
        # 현재 읽고 있는 티어 신호(KV 오프로드 쪽에서 get_offloader().is_ssd_window()로 본다)
        self.io_window = IoWindow()

        # Copy stream for async H2D transfers
        self.copy_stream = torch.cuda.Stream()

        # Optional SSD tier: offloaded layers beyond the host budget live on
        # NVMe and are streamed into the static buffers each forward.
        self.ssd_tier: SsdTier | None = None
        self.host_budget_bytes = 0
        self.host_tier_bytes = 0
        self.ssd_tier_bytes = 0
        if ssd_path:
            rank = 0
            ...
            self.ssd_tier = SsdTier(
                root=ssd_path,
                transport=ssd_transport,
                io_threads=ssd_io_threads,
                copy_stream=self.copy_stream,
                rank=rank,
                ring_mb=ssd_ring_mb,
            )
            self.ssd_tier.on_activity = self.io_window.activity  # 실제 읽기 시작/끝 → SSD 구간 신호
            self.host_budget_bytes = int(host_mem_total_bytes() * host_fraction)
            logger.info(
                "[PrefetchOffloader] SSD tier at %s (transport=%s, io_threads=%d), "
                "host budget %.2f GiB (%.0f%% of DRAM)",
                self.ssd_tier.root,
                ssd_transport,
                ssd_io_threads,
                self.host_budget_bytes / 2**30,
                host_fraction * 100,
            )
    ...
    def _whitelist(self, module: nn.Module) -> list[str]:
        if self.offload_params:
            return [name for name, _ in module.named_parameters()
                    if any(f".{p}." in f".{name}." for p in self.offload_params)]
        return [name for name, _ in module.named_parameters()]

    def _interleave_plan(self, nbytes: int) -> set[int] | None:
        """VLLM_OFFLOAD_TIER_LAYOUT=interleave: host layer 수는 block 배치와 같게(예산 // layer 크기),
        위치는 전체 오프로드 layer에 고르게. layer 크기가 같다고 보고 첫 layer 크기로 계산한다.
        모듈 생성기를 미리 펼치면 layer별 GPU→host 이동이 늦어져 적재 단계에서 GPU가 넘치므로,
        총 layer 수는 vLLM config에서 얻는다."""
        if getattr(self, "_il_plan_done", False):
            return self._il_plan
        self._il_plan_done = True
        self._il_plan = None
        layout = os.environ.get("VLLM_OFFLOAD_TIER_LAYOUT", "block").lower()
        if layout not in ("interleave", "interleave_ssd") or nbytes <= 0:
            return None
        try:
            from vllm.config import get_current_vllm_config
            cfg = get_current_vllm_config()
            n_layers = int(cfg.model_config.hf_text_config.num_hidden_layers)
        except Exception:
            logger.warning("[PrefetchOffloader] interleave: layer 수를 config에서 못 얻어 block 배치로 진행")
            return None
        n = sum(1 for i in range(n_layers) if i % self.group_size >= self.group_size - self.num_in_group)
        n_host = min(n, int(self.host_budget_bytes // nbytes))
        if layout == "interleave_ssd":
            # SSD layer를 비례로 흩음: floor((i+1)·n_ssd/n) > floor(i·n_ssd/n) 인 i가 SSD.
            # host가 셋 이상 연속되는 구간을 없애 2슬롯 파이프라인에서 SSD 읽기 하나(0.48 s)에
            # host 복사 둘(0.2 s×2)이 붙게 한다.
            n_ssd = n - n_host
            ssd_pos = {i for i in range(n) if (i + 1) * n_ssd // n > i * n_ssd // n}
            self._il_plan = set(range(n)) - ssd_pos
        else:
            self._il_plan = {int((k + 0.5) * n / n_host) for k in range(n_host)} if n_host else set()
        logger.info("[PrefetchOffloader] tier layout %s: host %d / ssd %d of %d layers (layer %.2f GiB), ssd positions %s",
                    layout, n_host, n - n_host, n, nbytes / 2**30, sorted(set(range(n)) - self._il_plan))
        return self._il_plan

    def _layer_mode(self, module: nn.Module, whitelist: list[str]) -> str:
        """Place a layer in the host tier while the budget lasts, else SSD."""
        if self.ssd_tier is None:
            return self.mode
        params = dict(module.named_parameters())
        nbytes = sum(
            params[n].numel() * params[n].element_size() for n in whitelist
        )
        plan = self._interleave_plan(nbytes)
        if plan is not None:
            if len(self.module_offloaders) in plan and self.host_tier_bytes + nbytes <= self.host_budget_bytes:
                self.host_tier_bytes += nbytes
                return "cpu"
            self.ssd_tier_bytes += nbytes
            return "ssd"
        if self.host_tier_bytes + nbytes <= self.host_budget_bytes:
            self.host_tier_bytes += nbytes
            return "cpu"
        self.ssd_tier_bytes += nbytes
        return "ssd"
    ...
    def wrap_modules(
        ...
        for module_index, module in enumerate(modules_generator):
            ...
            if module_index % self.group_size >= self.group_size - self.num_in_group:
                ...
                mode = self._layer_mode(module, whitelist)
    ...
    def _hook_module_forward(self, index: int, module: nn.Module):
        ...
        def forward(*args, **kwargs):
            ...
            # Start prefetch for the next layer(s) per the static plan
            # (see _build_prefetch_plan for the pass-boundary slot rule).
            ...
            for next_index in self._prefetch_plan[index]:
                torch.ops.vllm.start_prefetch(out0, next_index)
    ...
    def _build_prefetch_plan(self) -> list[list[int]]:
        """Which layers each layer's forward starts prefetching.

        Layer ``i`` normally starts ``i + prefetch_step``.  Across the pass
        boundary the target wraps to ``j = i + step - n``, which is only safe
        when no later layer of the current pass still has to run on the same
        slot (``r % step == j % step`` for some ``r`` in ``(i, n)``).  That
        happens whenever ``n % step != 0``; those wrapped targets are deferred
        to the last layer so the slot is free.  For ``n % step == 0`` the plan
        is identical to the plain circular schedule.
        """
        n = len(self.module_offloaders)
        ...
        for i in range(n):
            ...
            if tgt < n:
                ...
                continue
            j = tgt - n
            if any(r % step == j % step for r in range(i + 1, n)):
                deferred.append(j)
            else:
                plan[i].append(j)
        # deferred (wrapped) targets first: they are the next pass's first layers
        plan[n - 1] = deferred + plan[n - 1]
        if deferred:
            logger.info_once(
                "[PrefetchOffloader] %d modules not divisible by prefetch_step=%d: "
                "wrapped prefetch of layers %s deferred to the last layer "
                "(slot collision at the pass boundary)",
                n,
                step,
                str(deferred),
            )
    ...
    def _wait_for_layer(self, layer_idx: int):
        ...
        # SSD-tier reads are host-driven: block until the IO job has
        # recorded _copy_done_event (no-op for host-tier layers).
        offloader.wait_host()
    ...
    def _start_prefetch(self, layer_idx: int):
        ...
        offloader = self.module_offloaders[layer_idx]
        # 읽기를 내보내기 직전에 티어를 알린다. 이 지점이 실제 NVMe 트래픽보다
        # prefetch_step 만큼 앞서므로, KV 쓰기가 미리 비켜설 시간이 생긴다.
        self.io_window.mark(offloader.mode)
        torch.cuda.nvtx.mark(f"prefetch:L{layer_idx}:{offloader.mode}")
        offloader.start_onload_to_static()

    def is_ssd_window(self) -> bool:
        """True while weight reads from the SSD tier are in flight (plus a short grace)."""
        return self.io_window.is_ssd_window()
    ...
    def post_init(self):
        ...
        self._prefetch_plan = self._build_prefetch_plan()

        if self.ssd_tier is not None:
            ssd_buffers = [
                o._gpu_buffer
                for m in self.module_offloaders
                for o in m._param_offloaders.values()
                if isinstance(o, _SsdParamOffloader) and o._gpu_buffer is not None
            ]
            self.ssd_tier.register_buffers(ssd_buffers)
        ...
        if self.ssd_tier is not None:
            n_ssd = sum(1 for o in self.module_offloaders if o.mode == "ssd")
            logger.info(
                "[PrefetchOffloader] tiers: host %d layers / %.2f GiB "
                "(budget %.2f GiB), ssd %d layers / %.2f GiB in %d files",
                len(self.module_offloaders) - n_ssd,
                self.host_tier_bytes / 2**30,
                self.host_budget_bytes / 2**30,
                n_ssd,
                self.ssd_tier_bytes / 2**30,
                len(self.ssd_tier.files),
            )

        if self.ssd_tier is not None and self.ssd_tier.transport == "cufile":
            # cuFile은 첫 읽기 때 GPU bounce 캐시를 잡는다. vLLM이 GPU KV 크기를 정하기 전에 잡아 두어야
            # 그 몫이 KV 예산에서 빠진다. 없으면 기본 설정에서 첫 prefill이 OOM으로 죽는다.
            seen: set[int] = set()
            items = []
            for m in self.module_offloaders:
                if m.mode != "ssd":
                    continue
                for o in m._param_offloaders.values():
                    if isinstance(o, _SsdParamOffloader) and o.ssd_file is not None and o._gpu_buffer is not None \
                            and o._gpu_buffer.data_ptr() not in seen:
                        seen.add(o._gpu_buffer.data_ptr())
                        items.append((o.ssd_file, o._gpu_buffer))
            if items:
                futs = [self.ssd_tier.pool.submit(self.ssd_tier._read_one, rec, buf) for rec, buf in items]
                for f in futs:
                    f.result()
                torch.cuda.synchronize()
                logger.info("[PrefetchOffloader] cuFile warm-up: %d reads into static buffers", len(items))
...
class _ModuleOffloader:
    ...
    def __init__(
        ...
        # Pending SSD read job for this layer (see start_onload_to_static)
        self._ssd_future: Future | None = None
        ...
        for name in whitelist_param_names:
            p = param_dict[name]
            param_mode = mode
            if mode == "ssd" and p.numel() * p.element_size() < SSD_MIN_PARAM_BYTES:
                param_mode = "cpu"
            self._param_offloaders[name] = _BaseParamOffloader.create(
                param_mode,
                module=module,
                param_name=name,
                ssd_tier=ssd_tier,
    ...
    def wait_host(self) -> None:
        """Block until this layer's SSD read job (if any) has finished."""
        if self._ssd_future is not None:
            fut, self._ssd_future = self._ssd_future, None
            fut.result()
    ...
    def start_onload_to_static(self):
        ...
        # A previous read job for this slot must be over before reusing it.
        self.wait_host()
        ...
        if self._prefetch_in_capture and self.mode == "ssd":
            raise RuntimeError(
                "SSD-tier weight offload is host-driven and cannot be captured "
                "into a CUDA graph; run with enforce_eager=True."
            )
        ...
        ssd_items: list[tuple[SsdFile, torch.Tensor]] = []
        with torch.cuda.stream(self.copy_stream):
            for name, offloader in self._param_offloaders.items():
                gpu_buffer = offloader._gpu_buffer
                assert gpu_buffer is not None, "GPU buffer not assigned"
                if isinstance(offloader, _SsdParamOffloader):
                    ssd_items.append((offloader.ssd_file, gpu_buffer))
                    continue
                cpu_storage = offloader._cpu_storage
                assert cpu_storage is not None, "CPU storage not initialized"
                ...
                torch.cuda.nvtx.range_push(f"weight_h2d L{self.layer_idx} {gpu_buffer.numel() * gpu_buffer.element_size() >> 20}MiB")
                gpu_buffer.copy_(cpu_storage, non_blocking=True)
                torch.cuda.nvtx.range_pop()

        if ssd_items:
            # The IO job records _copy_done_event on copy_stream once every
            # file of this layer has landed in its GPU buffer (stream-ordered
            # after the host-tier copies enqueued above).
            assert self.ssd_tier is not None
            self._ssd_future = self.ssd_tier.submit_layer(
                ssd_items, fork_event, self._copy_done_event
            )
...
class _CpuParamOffloader(_BaseParamOffloader):
    ...
    def _offload_to_cpu_internal(self):
        ...
        self._cpu_storage = _empty_host(
            param.data.size(), param.data.stride(), param.data.dtype,
            param.data.layout, pin_memory,
        )
...
class _SsdParamOffloader(_CpuParamOffloader):
    """Offload parameter to a file on the SSD tier.

    During weight loading the parameter is backed by a shared file mapping,
    so the loader writes straight to disk (no host-side copy of the weight).
    After post_init the mapping is dropped and the page cache released; each
    forward reads the file into the GPU static buffer via SsdTier.
    """
    ...
    def __init__(
        ...
        module: nn.Module,
        param_name: str,
        ssd_tier: SsdTier | None,
        layer_idx: int,
    ):
        assert ssd_tier is not None, "SSD tier not configured"
        self._tier = ssd_tier
        self._layer_idx = layer_idx
        self.ssd_file: SsdFile | None = None
        super().__init__(module, param_name)

    def _offload_to_cpu_internal(self):
        param = self._param
        assert param.data.is_contiguous(), "SSD tier needs contiguous params"
        storage, rec = self._tier.new_file_tensor(
            self._layer_idx,
            self._param_name,
            param.data.shape,
            param.data.stride(),
        ...
        # No copy of the (uninitialized) GPU data: the weight loader fills
        # the mapping. Reassigning frees the GPU tensor.
        self._cpu_storage = storage
        self.ssd_file = rec
        self.offloaded_bytes = rec.nbytes
        param.data = storage

    def _update_cpu_storage_from_param(self) -> None:
        param = self._param
        assert self._cpu_storage is not None
        if param.data is self._cpu_storage:
            return
        # device_loading_context / quantization replaced the tensor: write
        # the final weights back into the file mapping.
        self._cpu_storage.copy_(param.data.to("cpu"))
        param.data = self._cpu_storage

    def assign_static_buffer(self, gpu_buffer: torch.Tensor) -> None:
        super().assign_static_buffer(gpu_buffer)
        assert self.ssd_file is not None
        self._tier.finalize(self.ssd_file)
        # Drop the mapping: the file is the tier from here on.
        self._cpu_storage = None
