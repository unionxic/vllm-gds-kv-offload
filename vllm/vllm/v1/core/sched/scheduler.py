...
from vllm.v1.kv_offload.split_policy import LAST_SPLIT_STATS, nvtx_mark
...
class Scheduler(SchedulerInterface):
    def __init__(
        ...
        # Split-source KV (see vllm/v1/kv_offload/split_policy.py).
        # `_kv_split_pending`: admitted split requests whose tail load is in
        # flight. `_kv_split_ready`: tail has landed, not yet applied.
        # `_kv_split_wait_since`: when the head finished ahead of the tail.
        self._kv_split_pending: set[str] = set()
        self._kv_alloc_fail_count: dict[str, int] = {}  # 진단용: 요청별 연속 할당 실패 수
        self._kv_split_ready: set[str] = set()
        self._kv_split_wait_since: dict[str, float] = {}
        # Wave gate for async KV loads (opt-in, see envs.VLLM_KV_LOAD_WAVE_GATE).
        self._kv_wave_gate: int = envs.VLLM_KV_LOAD_WAVE_GATE
        self._kv_wave_wait_s: float = envs.VLLM_KV_LOAD_WAVE_WAIT_S
        self._kv_wave_ready_at: dict[str, float] = {}
        self._kv_wave_hold_since: float | None = None
        # Async loads admitted in the current scheduling pass live in a local
        # queue until the pass ends, so the in-flight scan cannot see them.
        self._kv_loads_started_this_step: int = 0
        # Connector lookups that answered "not yet" in this pass: those
        # requests are wave-mates about to load, so level 2 must wait for them.
        self._kv_lookups_pending_this_step: int = 0
        # Requests whose async KV load was started and not yet promoted or
        # finished. Tracked by id so the check does not depend on which queue
        # (waiting, skipped_waiting, or the per-pass local queue) holds them.
        self._kv_inflight_load_ids: set[str] = set()
    ...
    def schedule(self, throttle_prefills: bool = False) -> SchedulerOutput:
        ...
        while req_index < len(self.running) and token_budget > 0:
            ...
            if request.kv_split_head_tokens:
                # Split-source KV: adopt the tail if its load has landed.
                self._maybe_apply_kv_split_tail(request)
            ...
            if request.kv_split_head_tokens:
                # Still waiting on the tail: never schedule past the boundary.
                # At the boundary this yields 0 and the request is skipped
                # (held) until the load finishes.
                num_new_tokens = max(
                    0,
                    min(
                        num_new_tokens,
                        request.kv_split_boundary - request.num_computed_tokens,
                    ),
                )
                ...
                while True:
                    ...
                    if self.policy == SchedulingPolicy.PRIORITY:
                        candidates = [
                            ...
                            if r.request_id not in self._kv_split_pending
                        ]
                        if not candidates:
                            break
                        preempted_req = max(
                        ...
                        if preempted_req is None:
                            # Only requests with an in-flight split tail load
                            # remain; preempting them would free blocks the
                            # load is writing to.
                            break
        ...
        if not preempted_reqs and self._pause_state == PauseState.UNPAUSED:
            ...
            self._kv_loads_started_this_step = 0
            self._kv_lookups_pending_this_step = 0
            ...
            while (self.waiting or self.skipped_waiting) and token_budget > 0:
                ...
                if request.num_computed_tokens == 0:
                    ...
                    if self.connector is not None:
                        ...
                        if ext_tokens is None:
                            ...
                            request_queue.pop_request()
                            self._kv_lookups_pending_this_step += 1
                            step_skipped_waiting.prepend_request(request)
                            continue
                    ...
                    # Split-source KV: the connector reported only the tail as
                    # externally available and recorded a head the GPU must
                    # recompute. The request prefills the head right away
                    # instead of parking in WAITING_FOR_REMOTE_KVS, and the
                    # tail load runs concurrently with that prefill.
                    split_head_tokens = request.kv_split_head_tokens
                    if split_head_tokens:
                        assert num_external_computed_tokens > 0
                        load_kv_async = False

                    # Total computed tokens (local + external).
                    # For a split request the head is not computed yet and the
                    # tail only becomes computed once its load lands, so only
                    # the local prefix cache hit counts here.
                    num_computed_tokens = num_new_local_computed_tokens + (
                        0 if split_head_tokens else num_external_computed_tokens
                    )
                ...
                external_load_encoder_input = []
                new_encoder_compute_budget = encoder_compute_budget
                # Wave gate level 2 (after the connector lookup so wave-mates'
                # own loads still get started): a fresh request that needs no
                # KV load would otherwise prefill alone while its wave-mates
                # are loading, splitting the wave into extra forwards.
                if (
                    self._kv_wave_gate >= 2
                    and not load_kv_async
                    and num_external_computed_tokens == 0
                    and request.num_computed_tokens == 0
                    and self._kv_wave_hold_fresh()
                ):
                    request_queue.pop_request()
                    step_skipped_waiting.prepend_request(request)
                    continue
                ...
                if load_kv_async:
                    ...
                    if split_head_tokens:
                        # Never prefill past the boundary: [boundary, boundary+
                        # tail) is being written by the load right now.
                        num_new_tokens = min(num_new_tokens, split_head_tokens)
                ...
                reserved_blocks = 0
                if load_kv_async or split_head_tokens:
                ...
                new_blocks = self.kv_cache_manager.allocate_slots(
                    ...
                    # A split request's tail blocks are allocated but still
                    # empty; caching them would publish garbage to the prefix
                    # cache. They are cached when the tail lands
                    # (_maybe_apply_kv_split_tail).
                    delay_cache_blocks=load_kv_async or bool(split_head_tokens),
                ...
                if new_blocks is None:
                    ...
                    # 진단: 같은 요청의 할당 실패가 계속되면(예: 적재 대기 요청의 예약 블록이
                    # 남아 새 요청이 영영 admit되지 않는 경우) 상태를 남긴다. 72B chunk 4096에서
                    # 외부 적중 요청이 매 step lookup만 반복한 정체의 원인 추적용.
                    n_fail = self._kv_alloc_fail_count.get(request_id, 0) + 1
                    self._kv_alloc_fail_count[request_id] = n_fail
                    if load_kv_async:
                        # 비동기 적재 admit은 (여유 − 다른 in-flight 요청의 예약)에 들어가야 한다.
                        # 예약을 쥔 요청들이 이 요청 뒤에 줄 서 있으면 여기서 break 하는 순간
                        # 그들도 영영 스케줄되지 않아 예약이 풀리지 않는다(head-of-line 교착.
                        # 72B chunk 4096에서 6시간 정체). 이 요청만 건너뛰고 뒤를 계속 본다.
                        request_queue.pop_request()
                        step_skipped_waiting.prepend_request(request)
                        continue
                    if n_fail in (20, 200) or n_fail % 2000 == 0:
                        logger.warning(
                            "allocate_slots failed %d times for %s: new=%d ext=%d local=%d "
                            "free_blocks=%s reserved=%d inflight_prefills=%d split_pending=%d "
                            "running=%d waiting=%d load_async=%s",
                            n_fail, request_id, num_new_tokens, num_external_computed_tokens,
                            num_new_local_computed_tokens,
                            getattr(self.kv_cache_manager.block_pool, "get_num_free_blocks", lambda: "?")(),
                            reserved_blocks, len(self._inflight_prefills), len(self._kv_split_pending),
                            len(self.running), len(self.waiting), load_kv_async,
                        )
                    break
                ...
                self._kv_alloc_fail_count.pop(request_id, None)
                if self.connector is not None:
                ...
                if load_kv_async:
                    ...
                    self._kv_loads_started_this_step += 1
                    self._kv_inflight_load_ids.add(request.request_id)
                    step_skipped_waiting.prepend_request(request)
                ...
                if split_head_tokens:
                    # The tail load is in flight while this request prefills
                    # its head. Counted as an arriving KV load for the wave
                    # gate (it holds other fresh requests), but the request
                    # itself never parks, so the gate never defers it.
                    self._kv_split_pending.add(request_id)
                    self._kv_loads_started_this_step += 1
                    self._inflight_prefills.add(request)
                    if self.needs_kv_cache_zeroing:
                        # The load will overwrite these blocks; zeroing them
                        # now would race the write.
                        self._skip_zero_block_ids.update(
                            self.kv_cache_manager.get_zeroing_block_ids_in_range(
                                request_id,
                                request.kv_split_boundary,
                                request.kv_split_boundary
                                + request.kv_split_tail_tokens,
                            )
                        )
    ...
    def _preempt_request(self, request: Request, timestamp: float) -> None:
        ...
        assert request.request_id not in self._kv_split_pending, (
            "a split request with an in-flight tail load must not be preempted"
        )
        self._clear_kv_split_state(request)
    ...
    def finish_requests(
        ...
        for request in valid_requests:
            delay_free_blocks = request.request_id in self._kv_split_pending
            if delay_free_blocks:
                # A split tail load is still writing into this request's
                # blocks; _update_from_kv_xfer_finished frees them.
                self._kv_split_ready.discard(request.request_id)
                self._kv_split_wait_since.pop(request.request_id, None)
            if request.status == RequestStatus.WAITING_FOR_REMOTE_KVS:
                self._kv_inflight_load_ids.discard(request.request_id)
                delay_free_blocks = (
    ...
    def _maybe_apply_kv_split_tail(self, request: Request) -> bool:
        """Adopt a split request's loaded tail once the head prefill reached
        the boundary and the connector reported the load finished.

        Semantics: while the tail is outstanding the request stays RUNNING and
        keeps its blocks; the scheduler simply caps its chunk at
        `kv_split_boundary` (0 new tokens once it gets there, so the request is
        skipped that step). When the load lands, `num_computed_tokens` jumps
        over the tail and the whole prefix is committed to the prefix cache.
        """
        req_id = request.request_id
        if request.num_computed_tokens < request.kv_split_boundary:
            return False
        if req_id not in self._kv_split_ready:
            # Head done, tail not here yet: hold (0 tokens scheduled).
            self._kv_split_wait_since.setdefault(req_id, time.monotonic())
            return False

        self._kv_split_ready.discard(req_id)
        self._kv_split_pending.discard(req_id)
        self._kv_wave_ready_at.pop(req_id, None)
        waited_since = self._kv_split_wait_since.pop(req_id, None)
        if waited_since is not None:
            LAST_SPLIT_STATS["tail_wait_events"] += 1
            LAST_SPLIT_STATS["tail_wait_s_total"] += time.monotonic() - waited_since

        if req_id in self.failed_recving_kv_req_ids:
            # The tail load failed; _update_requests_with_invalid_blocks
            # already rolled num_computed_tokens back to the valid prefix.
            self.failed_recving_kv_req_ids.discard(req_id)
        else:
            request.num_computed_tokens = (
                request.kv_split_boundary + request.kv_split_tail_tokens
            )
            self.kv_cache_manager.cache_blocks(request, request.num_computed_tokens)
            # On a full prompt hit the last token must still be recomputed to
            # produce the logits that sample the next token.
            if request.num_computed_tokens == request.num_tokens:
                request.num_computed_tokens = request.num_tokens - 1

        nvtx_mark(f"kv_tail_ready {req_id}")
        request.kv_split_head_tokens = 0
        request.kv_split_tail_tokens = 0
        request.kv_split_boundary = 0
        return True

    def _clear_kv_split_state(self, request: Request) -> None:
        req_id = request.request_id
        self._kv_split_pending.discard(req_id)
        self._kv_split_ready.discard(req_id)
        self._kv_split_wait_since.pop(req_id, None)
        request.kv_split_head_tokens = 0
        request.kv_split_tail_tokens = 0
        request.kv_split_boundary = 0

    def _pop_preemption_victim(self) -> Request | None:
        """Last running request that is safe to preempt.

        A split request with an in-flight tail load must not be preempted: the
        load is writing into blocks that preemption would hand back to the
        pool.
        """
        for i in range(len(self.running) - 1, -1, -1):
            if self.running[i].request_id not in self._kv_split_pending:
                return self.running.pop(i)
        return None

    def _kv_loads_in_flight(self) -> bool:
        if self._kv_loads_started_this_step > 0 or self._kv_lookups_pending_this_step > 0:
            return True
        if self._kv_split_pending:
            return True
        if any(
            rid not in self.finished_recving_kv_req_ids
            for rid in self._kv_inflight_load_ids
        ):
            return True
        return any(
            r.status == RequestStatus.WAITING_FOR_REMOTE_KVS
            and r.request_id not in self.finished_recving_kv_req_ids
            for q in (self.waiting, self.skipped_waiting)
            for r in q
        )

    def _kv_wave_hold_fresh(self) -> bool:
        """Level-2 gate: hold fresh (no-load) requests while wave-mates load,
        bounded by VLLM_KV_LOAD_WAVE_WAIT_S from the first hold."""
        if not self._kv_loads_in_flight():
            self._kv_wave_hold_since = None
            return False
        now = time.monotonic()
        if self._kv_wave_hold_since is None:
            self._kv_wave_hold_since = now
        return (now - self._kv_wave_hold_since) <= self._kv_wave_wait_s

    def _kv_wave_gate_defers(self, request: Request) -> bool:
        """Opt-in: keep a load-complete request parked while other admitted
        requests' async KV loads are still in flight.

        Without this, the first request whose KV lands runs its prefill alone
        and the rest of the wave joins one or two forwards later; in a
        weight-streaming deployment every forward costs a full weight pass, so
        that fragmentation is the dominant cost of loading KV from storage.
        The hold is bounded by VLLM_KV_LOAD_WAVE_WAIT_S from the moment this
        request's own load finished, so a stuck peer cannot stall it.
        """
        if not self._kv_wave_gate:
            return False
        now = time.monotonic()
        ready_at = self._kv_wave_ready_at.setdefault(request.request_id, now)
        if now - ready_at > self._kv_wave_wait_s:
            self._kv_wave_ready_at.pop(request.request_id, None)
            return False
        if self._kv_loads_in_flight():
            return True
        self._kv_wave_ready_at.pop(request.request_id, None)
        return False
    ...
    def _try_promote_blocked_waiting_request(self, request: Request) -> bool:
        ...
        if request.status == RequestStatus.WAITING_FOR_REMOTE_KVS:
            ...
            if request.request_id not in self.finished_recving_kv_req_ids:
                return False
            if self._kv_wave_gate_defers(request):
                return False
        ...
        * if finished_sending: free the blocks
        ...
        for req_id in kv_connector_output.finished_recving or ():
            ...
            if req.status == RequestStatus.WAITING_FOR_REMOTE_KVS:
                ...
                # Blocks were held back for the in-flight load; free them now.
                self._kv_split_pending.discard(req_id)
                self._kv_split_ready.discard(req_id)
                self._kv_split_wait_since.pop(req_id, None)
                self._free_blocks(req)
            else:
                # Split-source KV: the request is RUNNING, prefilling its head
                # while this load filled the tail.
                assert req_id in self._kv_split_pending, (
                    f"unexpected finished_recving for {req_id} "
                    f"in state {req.status.name}"
                )
                self._kv_split_pending.discard(req_id)
                self._kv_split_ready.add(req_id)
