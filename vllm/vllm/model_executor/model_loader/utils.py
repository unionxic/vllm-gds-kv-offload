...
def device_loading_context(module: torch.nn.Module, target_device: torch.device):
    ...
    for name, p in module.named_parameters():
        if p.device.type == "cpu":
            ...
            # Keep the original storage (pinned host / file-backed mapping of
            # the weight offloader) so it can be refilled in place instead of
            # allocating a second pageable copy of every offloaded weight.
            original_tensors[name] = p.data
    ...
    try:
        ...
        for name, p in module.named_parameters():
            if name in original_device_states:
                ...
                if (
                    p.data.shape == orig.shape
                    and p.data.dtype == orig.dtype
                    and p.data.stride() == orig.stride()
                ):
                    orig.copy_(p.data)
                    p.data = orig
                else:
                    # Processing changed the layout (e.g. quantization repack)
