...
class BaseOffloader(ABC):
    ...
    def is_ssd_window(self) -> bool:
        """True while weights are being streamed from the SSD tier.

        KV offload transports check this (via ``get_offloader()``) to yield the
        NVMe to weight reads.  Offloaders without an SSD tier never do SSD IO,
        so the base implementation is always False.
        """
        return False
...
def create_offloader(offload_config: "OffloadConfig") -> BaseOffloader:
    ...
    if backend == "prefetch":
        return PrefetchOffloader(
            ...
            mode="cpu",
            ssd_path=prefetch.offload_ssd_path,
            host_fraction=prefetch.offload_host_fraction,
            ssd_transport=prefetch.offload_ssd_transport,
            ssd_io_threads=prefetch.offload_ssd_io_threads,
            ssd_ring_mb=prefetch.offload_ssd_ring_mb,
        )
    elif backend == "uva":
        return UVAOffloader(
