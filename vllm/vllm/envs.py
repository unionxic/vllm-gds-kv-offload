...
if TYPE_CHECKING:
    ...
    VLLM_OFFLOAD_PIN_EXACT: bool = False
    VLLM_KV_LOAD_WAVE_GATE: int = 0
    VLLM_KV_LOAD_WAVE_WAIT_S: float = 30.0
...
environment_variables: dict[str, Callable[[], Any]] = {
    ...
    # Pin offloaded host tier with cudaHostRegister on exact-size pageable
    # buffers instead of the caching host allocator (which rounds each block
    # up to a power of two, ~1.45x footprint for OPT-66B layers).
    "VLLM_OFFLOAD_PIN_EXACT": lambda: bool(
        int(os.getenv("VLLM_OFFLOAD_PIN_EXACT", "0"))
    ),
    # 0: off. 1: hold a request whose async KV load has finished while other
    # requests' loads are still in flight. 2: additionally hold fresh requests
    # that need no load, so a mixed wave (hits + misses) prefills in one
    # forward. Both bounded by *_WAIT_S seconds.
    "VLLM_KV_LOAD_WAVE_GATE": lambda: int(os.getenv("VLLM_KV_LOAD_WAVE_GATE", "0")),
    "VLLM_KV_LOAD_WAVE_WAIT_S": lambda: float(
        os.getenv("VLLM_KV_LOAD_WAVE_WAIT_S", "30")
    ),
