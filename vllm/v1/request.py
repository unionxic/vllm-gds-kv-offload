...
class Request:
    def __init__(
        ...
        # Split-source KV (vllm/v1/kv_offload/split_policy.py). Set by the
        # offloading connector at admission: the first `kv_split_head_tokens`
        # tokens of the offload hit are recomputed on the GPU while
        # `kv_split_tail_tokens` tokens starting at `kv_split_boundary` are
        # loaded from the tiers. 0 means no split (load everything).
        self.kv_split_head_tokens = 0
        self.kv_split_tail_tokens = 0
        self.kv_split_boundary = 0
