...
class OffloadingManager(ABC):
    ...
    def key_tiers(
        ...
        """Storage tier holding each key: "host" or "ssd".

        Used by the split-source KV policy to price a load. The default
        assumes the fastest tier, which keeps the policy conservative
        (little or no head recompute) for managers that do not report.
        """
        return ["host"] * len(keys)
