...
from vllm.v1.kv_offload.split_policy import (  # noqa: F401  (re-exported)
    LAST_SPLIT_STATS,
    SplitContext,
    SplitPolicy,
    nvtx_mark,
...
class RequestOffloadState:
    ...
    # Split-source KV: tokens of the hit prefix the GPU recomputes (head).
    # The tail, which is loaded, starts at
    # num_locally_computed_tokens + split_head_tokens. 0 means no split.
    split_head_tokens: int = 0
...
class OffloadingConnectorScheduler:
    ...
    def __init__(
        ...
        # --- split-source KV (head recompute + tail load) ---
        self._split_policy = SplitPolicy.from_env()
        # KV bytes of one offloaded chunk, per group.
        self._split_chunk_bytes: tuple[int, ...] = tuple(
            kv_cache_config.kv_cache_groups[g.group_idx].kv_cache_spec.page_size_bytes
            * self.config.blocks_per_chunk
            for g in self.config.kv_group_configs
        )
        # Splitting only has a defined head/tail boundary for a single
        # full-attention group: the boundary must be one token position for
        # every group, and SWA/eagle/mamba groups load a suffix, not a prefix.
        only_group = self.config.kv_group_configs[0]
        self._split_eligible: bool = (
            self._split_policy.enabled
            and len(self.config.kv_group_configs) == 1
            and only_group.sliding_window_size_in_chunks is None
            and not only_group.is_eagle_group
            and self._mamba_align_size is None
        )
        if self._split_policy.enabled and not self._split_eligible:
            logger.warning(
                "KV split-source disabled: needs a single full-attention KV "
                "cache group (groups=%d)",
    ...
    def _decide_split(
        ...
        """Split the hit prefix into a recomputed head and a loaded tail.

        Returns the number of tokens to report as externally available (the
        tail). Records the head on the request so the core scheduler knows
        those tokens still have to be prefilled. Returns `num_hit_tokens`
        unchanged when no split is taken.
        """
        if not self._split_eligible or num_hit_tokens <= 0:
            return num_hit_tokens

        group_config = self.config.kv_group_configs[0]
        group_state = req_status.group_states[0]
        tokens_per_chunk = group_config.tokens_per_chunk
        hit_end = num_computed_tokens + num_hit_tokens
        if hit_end % tokens_per_chunk != 0:
            # The loader always ends on a chunk boundary; bail out otherwise.
            return num_hit_tokens

        # Chunk indices are absolute (chunk c covers tokens
        # [c*tokens_per_chunk, (c+1)*tokens_per_chunk)). The first hit chunk
        # may be partly covered by the GPU prefix cache; that is the same
        # chunk update_state_after_alloc starts the load from.
        first_chunk = num_computed_tokens // tokens_per_chunk
        end_chunk = hit_end // tokens_per_chunk
        num_hit_chunks = end_chunk - first_chunk
        if num_hit_chunks <= 0:
            return num_hit_tokens

        tiers = self.manager.key_tiers(
            group_state.offload_keys[first_chunk:end_chunk], req_status.req_context
        )
        ctx = SplitContext(
            chunk_bytes=self._split_chunk_bytes[0], req_id=request.request_id
        )
        k = self._split_policy.decide(num_hit_chunks, tokens_per_chunk, tiers, ctx)
        if k <= 0:
            return num_hit_tokens

        if self._split_policy.mode == "serial":
            # 대조군: 겹치지 않음. 앞 (H-k) 청크만 보통의 프리픽스 적중으로 보고하고
            # 뒤 k 청크는 적재가 끝난 뒤 스케줄러가 새 토큰으로 계산한다.
            # 적재 파티션이 hit chunk 수를 쓰므로 그 값도 줄인 적중에 맞춘다.
            # 적중 끝은 청크 경계여야 한다(첫 청크가 GPU 프리픽스 캐시에 일부 덮여 있어도).
            reported = (end_chunk - k) * tokens_per_chunk - num_computed_tokens
            if reported <= 0:
                req_status.update_num_hit_chunks(num_computed_tokens)
                return 0
            req_status.update_num_hit_chunks(num_computed_tokens + reported)
            return reported

        # The boundary is chunk-aligned, so the tail load stays chunk-aligned.
        boundary = (first_chunk + k) * tokens_per_chunk
        head_tokens = boundary - num_computed_tokens
        tail_tokens = hit_end - boundary
        assert head_tokens > 0
        if tail_tokens <= 0:
            # k == H: recompute the whole hit prefix, nothing left to load.
            return 0

        req_status.split_head_tokens = head_tokens
        request.kv_split_head_tokens = head_tokens
        request.kv_split_tail_tokens = tail_tokens
        request.kv_split_boundary = boundary
        nvtx_mark(
            f"kv_split {request.request_id} head={head_tokens} tail={tail_tokens}"
        )
        logger.debug(
            "Request %s split: recompute %d head tokens, load %d tail tokens",
            request.request_id,
            head_tokens,
            tail_tokens,
        )
    ...
    def get_num_new_matched_tokens(
        ...
        # This can be called again for the same request (deferred lookup,
        # re-admission after preemption): start from a clean split state.
        req_status.split_head_tokens = 0
        ...
        if num_hit_tokens:
            # May shrink the reported count to the tail and record the head
            # on the request (see _decide_split).
            num_hit_tokens = self._decide_split(
                request, req_status, num_computed_tokens, num_hit_tokens
            )
    ...
    def update_state_after_alloc(
        ...
        # Split-source KV: the recomputed head sits between the locally cached
        # prefix and the tokens being loaded, so it counts towards the cached
        # span (blocks are allocated for it) but not towards the load.
        split_head_tokens = req_status.split_head_tokens
        num_cached_tokens = (
            num_locally_computed_tokens + split_head_tokens + num_external_tokens
        )
        ...
        for group_config, group_state, group_blocks in zip(
            ...
            # The load starts after the head the GPU is about to recompute.
            # split_head_tokens is chunk-aligned, so load_start_block lands on
            # a chunk boundary and the first loaded chunk is not truncated.
            load_start_block = (
                num_locally_computed_gpu_blocks + split_head_tokens // tokens_per_block
            )
            num_pending_gpu_blocks = num_gpu_blocks - load_start_block
            ...
            if num_pending_gpu_blocks:
                start_chunk_idx = load_start_block // self.config.blocks_per_chunk
                keys_to_load.extend(offload_keys[start_chunk_idx:num_chunks])

            dst_block_ids.extend(
                block.block_id
                for block in group_blocks[load_start_block:num_gpu_blocks]
            )
            group_sizes.append(num_pending_gpu_blocks)
            block_indices.append(load_start_block)
        ...
        if split_head_tokens:
            # Counted here, not at the decision: a request may be looked up
            # several times before it is admitted, and only this call commits
            # the split by submitting the tail load.
            LAST_SPLIT_STATS["requests_split"] += 1
            LAST_SPLIT_STATS["head_tokens_recomputed_total"] += split_head_tokens
            LAST_SPLIT_STATS["tail_tokens_loaded_total"] += num_external_tokens
