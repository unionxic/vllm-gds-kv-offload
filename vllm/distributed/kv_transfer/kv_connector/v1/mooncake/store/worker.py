...
from .host_staging import HostStagingStore
...
class MooncakeStoreWorker:
    ...
    def __init__(
        ...
        # BAR1이 작아 KV 텐서를 RDMA에 등록하지 못하는 GPU용 host 경유 예산(GB).
        # 0이면 경유를 쓰지 않고 등록 실패를 그대로 둔다(기본 동작).
        self.host_staging_gb = float(extra_config.get("host_staging_gb", 0.0) or 0.0)
    ...
    def register_kv_caches(
        ...
        addrs: list[int] = []
        block_lens: list[int] = []
        # (base_addr, nbytes, storage). 등록이 실패해 host 경유로 넘어갈 때 쓴다.
        ...
        for value in kv_caches.values():
            ...
            regions.append((base_addr, region_len, cache_storage))
            ret = self.store.register_buffer(base_addr, region_len)
            if ret == 0:
                registered_addrs.append(base_addr)
            else:
        ...
        if register_failed and self.host_staging_gb > 0:
            # GPU BAR1이 KV 텐서보다 작다. 부분 등록을 되돌리고 pinned host 슬랩
            # 경유로 바꾼다. 원격 구간은 그대로 transfer engine RDMA.
            for base_addr in registered_addrs:
                try:
                    self.store.unregister_buffer(base_addr)
                except Exception:  # noqa: BLE001
                    pass
            num_slabs = 1 + max(1, envs.VLLM_MOONCAKE_LOAD_RECV_THREADS)
            slab_bytes = int(self.host_staging_gb * 2**30) // num_slabs
            self.store = HostStagingStore(
                self.store, regions, slab_bytes, num_slabs
            )
            logger.warning(
                "KV 텐서 RDMA 등록 실패(BAR1 부족) → host pinned 경유로 전환: "
                "슬랩 %d개 x %.2f GiB",
                num_slabs,
                slab_bytes / 2**30,
            )
            ...
            logger.error(
                "KV 텐서 RDMA 등록 실패. host_staging_gb가 0이라 그대로 진행한다 "
                "(이후 put/get이 TRANSFER_FAIL로 떨어진다)."
            )
